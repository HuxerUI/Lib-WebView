...
@class HUXMacOSWebViewNavigationDelegate;

namespace huxerui::detail {

struct MacOSWebViewInstance : std::enable_shared_from_this<MacOSWebViewInstance> {
  WKWebView* view = nil;
  HUXMacOSWebViewNavigationDelegate* navigation_delegate = nil;
...
@interface HUXMacOSWebViewNavigationDelegate : NSObject <WKNavigationDelegate, WKScriptMessageHandler> {
 @private
  std::weak_ptr<huxerui::detail::MacOSWebViewInstance> instance_;
}

- (instancetype)initWithInstance:(std::weak_ptr<huxerui::detail::MacOSWebViewInstance>)instance;
...
void EmitNavigationState(const std::shared_ptr<MacOSWebViewInstance>& instance, bool loading) {
...
WebViewNavigationState NavigationState(
    const std::shared_ptr<MacOSWebViewInstance>& instance,
    bool loading
) {
  WKWebView* view = instance->view;
  return {
...
void EmitStateEvent(const std::shared_ptr<MacOSWebViewInstance>& instance, bool loading) {
  if (!instance || instance->disposed || instance->view == nil) {
    return;
  }
...
void EmitLoadError(const std::shared_ptr<MacOSWebViewInstance>& instance, NSError* error, NSString* url) {
...
void LoadUrl(const std::shared_ptr<MacOSWebViewInstance>& instance, const std::string& value) {
  if (!instance || instance->disposed || instance->view == nil) {
    return;
  }
  ...
  std::weak_ptr<MacOSWebViewInstance> weak_instance = instance;
  dispatch_async(dispatch_get_main_queue(), ^{
    std::shared_ptr<MacOSWebViewInstance> current = weak_instance.lock();
...
void LoadHtml(const std::shared_ptr<MacOSWebViewInstance>& instance, const WebViewHtml& html) {
  if (!instance || instance->disposed || instance->view == nil) {
    return;
  }
...
bool RunCommand(
    const std::weak_ptr<MacOSWebViewInstance>& weak_instance,
    WebViewControllerCommand command
) {
  std::shared_ptr<MacOSWebViewInstance> instance = weak_instance.lock();
  if (!instance || instance->disposed || instance->view == nil) {
    return false;
  }
...
std::shared_ptr<MacOSWebViewInstance> CreateMacOSWebView(
    const WebViewProperties& properties,
    PlatformEventEmitter events
) {
...
#pragma clang diagnostic pop
  ...
  auto instance = std::make_shared<MacOSWebViewInstance>();
  instance->events = std::move(events);
  instance->navigation_delegate = [[HUXMacOSWebViewNavigationDelegate alloc] initWithInstance:instance];
...
void DisposeMacOSWebView(MacOSWebViewInstance& instance) {
...
void InstallPlatformWebView(RootContext& root) {
  macos::PlatformViewFactory<WebViewProperties, MacOSWebViewInstance, WebViewController> factory{
      .create = [](NSWindow*, const WebViewProperties& properties, PlatformEventEmitter events) {
        return CreateMacOSWebView(properties, std::move(events));
      },
      .view = [](const std::shared_ptr<MacOSWebViewInstance>& instance) -> NSView* {
        return instance->view;
      },
      .update = [](MacOSWebViewInstance& value, const WebViewProperties& properties) {
        ApplyProperties(value.shared_from_this(), properties, false);
      },
      .dispose = DisposeMacOSWebView,
      .connect = [](MacOSWebViewInstance& value, const WebViewController& controller) {
        std::weak_ptr<MacOSWebViewInstance> weak_instance = value.shared_from_this();
      ...
      .disconnect = [](MacOSWebViewInstance&, const WebViewController& controller) {
...
@implementation HUXMacOSWebViewNavigationDelegate
...
- (void)webView:(WKWebView*)webView
  ...
  std::shared_ptr<huxerui::detail::MacOSWebViewInstance> instance = instance_.lock();
